...
function TaskCardPresentation({
  ...
  return (
    <Card
      className={cn(
        "group relative ring-0 border border-border/50 bg-card/80 backdrop-blur-sm shadow-sm hover:shadow-md transition-all duration-200 hover:border-border gap-0",
        className,
      )}
    >
      <CardHeader
...
export function TaskCardPreview({ task, className }: TaskCardPreviewProps) {
  return (
    <TaskCardPresentation
      task={task}
      className={className}
      actions={
        <div className="h-6 w-6 p-0 inline-flex items-center justify-center rounded text-muted-foreground/70">
